const ALLOWED_TAGS = ['a', 'img', 'button', 'input', 'textarea', 'select', 'table', 'tr', 'td', 'th', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'ul', 'ol', 'li', 'code', 'pre'];
const ALLOWED_INPUT_TYPES = ['text', 'search', 'email', 'password', 'url', 'tel', 'checkbox', 'radio'];
...
export default function domToMarkdown(node: Node): string {
    ...
    function getSelectors(element: Element): string {
        ...
        if (element.className && typeof element.className === 'string') {
            const classes = element.className.trim().split(WHITESPACE_REGEX).filter(c => c);
            if (classes.length > 0) {
    ...
    while (currentNode) {
        ...
        switch (tagName) {
            ...
            case 'button':
                ...
                if (buttonText) {
                    const disabled = currentNode.hasAttribute('disabled') ? ' [DISABLED]' : '';
                    const selectors = getSelectors(currentNode);
            ...
            case 'input':
                ...
                if (ALLOWED_INPUT_TYPES.includes(inputType)) {
                    ...
                    if (inputType === 'checkbox') {
                        const checked = (currentNode as HTMLInputElement).checked ? ' [CHECKED]' : ' [UNCHECKED]';
                        ...
                        const checked = (currentNode as HTMLInputElement).checked ? ' [SELECTED]' : ' [UNSELECTED]';
            ...
            case 'textarea':
                const textareaPlaceholder = currentNode.getAttribute('placeholder') || '';
                const textareaValue = (currentNode as HTMLTextAreaElement).value || '';
                const textareaDisabled = currentNode.hasAttribute('disabled') ? ' [DISABLED]' : '';
                const textareaSelectors = getSelectors(currentNode);

                let textareaStr = '[TEXTAREA]';
                if (textareaPlaceholder) textareaStr += ` placeholder:"${textareaPlaceholder}"`;
            ...
            case 'select':
                const selectDisabled = currentNode.hasAttribute('disabled') ? ' [DISABLED]' : '';
                const selectMultiple = currentNode.hasAttribute('multiple') ? ' [MULTIPLE]' : '';
            ...
            case 'ol':
                ...
                if (!processedLists.has(currentNode)) {
                    const listStr = processList(currentNode as HTMLUListElement | HTMLOListElement);
                    if (listStr) {
...
function processList(list: HTMLUListElement | HTMLOListElement, depth = 0): string {
    ...
    listItems.forEach((item, index) => {
        ...
        const itemText = textNodes.map(node => {
            if (node.nodeType === Node.TEXT_NODE) {
                return node.textContent?.trim() || '';
            } else {
                return (node as Element).textContent?.trim() || '';
            }
        }).join(' ').replace(WHITESPACE_REGEX, ' ').trim();
        ...
        nestedLists.forEach(nestedList => {
            const nestedListStr = processList(nestedList as HTMLUListElement | HTMLOListElement, depth + 1);
            if (nestedListStr) {
