...
interface ConversationState {
    ...
    getConversations: (options: {
        limit: number;
        cursor?: number;
    }) => { conversations: Conversation[]; nextCursor?: number };
...
export const useConversationStore = createIndexDBStore<ConversationState>({
    ...
    handler: (set, get) => ({
        ...
        updateLastMessage: (message: Partial<Message>) => {
            ...
            const updatedMessages = [...conversation.messages.slice(0, -1), updatedMessage];
        ...
        getConversations: ({ limit, cursor }) => {
            ...
            if (!cursor) {
                ...
                return {
                    ...
                    nextCursor: paginatedConversations[paginatedConversations.length - 1]?.lastModified,
                };
            }

            const cursorIndex = sortedConversations.findIndex((c) => c.lastModified === cursor);
            if (cursorIndex === -1) {
                return { conversations: [], nextCursor: undefined };
            }

            const paginatedConversations = sortedConversations.slice(cursorIndex + 1, cursorIndex + 1 + limit);
        ...
        clearConversations: () => {
            set({ conversations: {}, currentConversationId: null, validationError: null });
        },
