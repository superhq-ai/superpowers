...
const Chat = () => {
    ...
    const handleSubmit = async (images?: File[]) => {
        if (!settings || !settings.selectedProvider || !settings.model || !settings.apiKeys[settings.selectedProvider]) {
            setValidationError("Please select a provider and model in settings, and ensure you have entered an API key.");
            return;
        }
...
Title:`;
                const titleLlmMessages: LLMMessage[] = [{ role: "user", content: titlePrompt }];
                try {
    ...
    return (
        <>
                        ...
                        <div className="text-center max-w-lg">
                            <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-left">
                                <div className="p-4 rounded-lg">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">"Find top 5 trending repositories on github"</p>
                                </div>
                                <div className="p-4 rounded-lg">
                                    <div className="flex items-center">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">I can search the web for real-time information.</p>
                                </div>
                                <div className="p-4 rounded-lg">
                                    <div className="flex items-center">
                                    ...
                                    <p className="text-xs text-gray-500 mt-1">I have access to this web browser to help you with your tasks.</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            {validationError && (
                <div className="px-4 py-3" role="alert">
                    <strong className="font-bold text-red-500">Error:</strong>
                    <span className="block sm:inline text-red-500"> {validationError}</span>
                </div>
            )}
